const tracer = require('dd-trace').init({
  ...
  version: '1.0.5' // <-- Use a real version
...
const { 
  addRequestId, 
  requestLogger, 
  // errorLogger is now largely redundant but kept in imports for now
  errorLogger, 
  securityLogger,
  logger // <-- NEW: Import the Winston logger instance
...
app.use((err, req, res, next) => {
  ...
  logger.error({
      ...
      reqId: req.reqId,
      // The logger will automatically inject dd.trace_id and dd.span_id here
  });

  // 🎯 Original error handling logic remains to send response to client
